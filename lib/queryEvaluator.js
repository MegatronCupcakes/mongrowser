...
export const queryEvaluator = (searchObject) => {    
    ...
    const _evaluationForKey = (_key, _searchObject, _parentKeys, _parentObject) => {
        ...
        const _getValues = (__searchObject, __testedRecord, __key, __parentKeys) => {
            ...
            let __searchValue = __searchObject[__key];
        ...
        switch (_key) {
            ...
            case '$all':
                return _testRecord => {
                    const [_searchValue, _testedValue] = _getValues(_searchObject, _testRecord, _key, _parentKeys);                    
                    return Array.isArray(_searchValue) && _searchValue.every(searchValue => _testedValue.includes(searchValue));
                }
            case '$elemMatch':
                return _testRecord => {
                    const [_searchValue, _testedValue] = _getValues(_searchObject, _testRecord, _key, _parentKeys);                    
                    const _consoleLog = _testedValue.some(_value => _value >= 85 && _value <= 90);                                        
                    const _searchField = Object.keys(_searchObject)[0];                    
                    return _testedValue.some(_value => {                        
                        return Object.keys(_searchValue).map(_searchKey => {                                 
                            const matchObject = {};                            
                            matchObject[_searchField] = {};
                            matchObject[_searchField][_searchKey] = _searchValue[_searchKey];                            
                            const _stubRecord = {};
                            _stubRecord[_searchField] = _value;
                            const evaluation = _evaluationForKey(_searchKey, matchObject, _parentKeys, _parentObject);
                            return evaluation(_stubRecord);
                        }).every(bool => bool);
                    });
                }
            case '$size':
                return _testRecord => {
                    const [_searchValue, _testedValue] = _getValues(_searchObject, _testRecord, _key, _parentKeys);                    
                    return Array.isArray(_testedValue) && _testedValue.length == _searchValue;
                }
